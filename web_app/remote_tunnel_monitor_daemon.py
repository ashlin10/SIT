...
RE_REMOTE_TIMESTAMP_PLAIN = re.compile(
...
RE_LOCAL_TIMESTAMP = re.compile(r'^\[(\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2})\]')


def parse_remote_timestamp(ts_str: str) -> Optional[datetime]:
    try:
...
def parse_local_timestamp(ts_str: str) -> Optional[datetime]:
    try:
...
def collect_remote_logs_in_window(filepaths: List[str], start_ts: datetime, end_ts: datetime) -> List[str]:
    collected = []
    for filepath in filepaths:
        for line in read_file_lines(filepath):
            ts = extract_remote_timestamp(line)
            if ts and start_ts <= ts <= end_ts:
                collected.append(line)
    return collected


def collect_local_logs_in_window(filepaths: List[str], start_ts: datetime, end_ts: datetime) -> List[str]:
    collected = []
    for filepath in filepaths:
        for line in read_file_lines(filepath):
            m = RE_LOCAL_TIMESTAMP.match(line)
            if m:
                ts = parse_local_timestamp(m.group(1))
                if ts:
                    # Make local timestamp timezone-aware (assume UTC) for proper comparison
...
class LocalTunnelMonitor:
    ...
    def _process_interval(self, interval_start: datetime, interval_end: datetime) -> None:
        ...
        remote_lines = collect_remote_logs_in_window(remote_files, interval_start, interval_end)
        entries_750007 = parse_750007_entries(remote_lines)
        ...
        if not entries_750007:
            ...
            logging.info("No 750007 events; appended empty report section.")
            self._write_count()
            return
        ...
        local_lines = collect_local_logs_in_window(local_files, window_start, window_end)
        remote_window_lines = collect_remote_logs_in_window(remote_files, window_start, window_end)
